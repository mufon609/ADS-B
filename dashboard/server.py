...
def read_status():
    ...
    try:
        ...
        logger.error(
            f"ERROR: Unexpected error reading status file '{path}': {e}")
...
def _rel_to_logs_url(path: str) -> Optional[str]:
    ...
    try:
        ...
        if os.path.commonpath([abs_path, abs_root]) != abs_root:
            logger.warning(
                f"Warning: Attempted to access path outside LOG_DIR: {path}")
            return None
        ...
        logger.warning(
            f"Warning: Error creating relative URL for path '{path}': {e}")
        return None
...
def _safe_read_json(path: str) -> Optional[dict]:
    ...
    try:
        ...
        logger.error(
            f"ERROR: Unexpected error reading JSON file '{path}': {e}")
        return None
...
def _find_sequence_dirs_for_icao(icao: str) -> List[str]:
    ...
    try:
        ...
        logger.warning(
            f"Warning: Error sorting sequence directories for {icao}: {e}")
        ...
        with os.scandir(STACK_ROOT) as icao_entries:
            for icao_entry in icao_entries:
                ...
                try:
                    ...
                    logger.warning(
                        f"Warning: Error scanning directory '{icao_dir}': {e}")
...
async def api_latest_stack():
    ...
    mtime_task = asyncio.to_thread(
        _sequence_mtime, seq_dir)  # Get mtime async too
...
async def api_recent_stacks(icao: str, limit: int = 5):
    ...
    abs_icao_dir = os.path.abspath(os.path.join(STACK_ROOT, icao_clean))
    if os.path.commonpath([abs_icao_dir, os.path.abspath(STACK_ROOT)]) != os.path.abspath(STACK_ROOT):
        return JSONResponse(
            content={"error": "forbidden",
                     "message": "Invalid ICAO path specified."},
            status_code=403,
        )

    limit = max(1, min(limit, 50))  # Clamp limit

    seq_dirs = await asyncio.to_thread(_find_sequence_dirs_for_icao, icao_clean)
    ...
    if tasks:
        try:
            ...
            if exceptions:
                logger.warning(
                    f"Warning: Errors occurred fetching recent stacks for {icao}: {exceptions}")
        except Exception as e:
...
async def api_sequence_manifest(icao: str, sequence_id: str):
    ...
    # Verify that the resolved path is within the STACK_ROOT directory
    ...
    if not os.path.isdir(seq_dir):
        return JSONResponse(
            content={"error": "not_found",
                     "message": f"Sequence directory not found: {seq_dir}"},
            headers={"Cache-Control": "no-store"},
            status_code=404,
        )
