...
def run_calibration():
    ...
    logger.info("NOTE: Point your telescope at a star-rich area of the night sky before running.")
    ...
    try:
        ...
        logger.info("Attempting to plate-solve the image (this may take up to 60 seconds)...")
        ...
        if os.path.exists(wcs_path):
            ...
            logger.error("Try pointing to a different area of the sky or increasing exposure time.")
            logger.error("Solver output:", result.stderr)

    except FileNotFoundError:
        logger.critical("\n--- ❌ Calibration Failed: 'astap' command not found. ---")
        logger.critical("Ensure ASTAP is installed and accessible in your system's PATH.")
