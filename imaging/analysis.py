...
def _load_fits_data(image_path: str) -> Optional[np.ndarray]:
    ...
    try:
        ...
        with fits.open(image_path, memmap=False) as hdul:
            ...
            if img_data is None:
                logger.warning(
                    f"Warning: No data found in primary HDU of {image_path}.")
                return None

        # Check dimensions AFTER confirming data is not None
        if img_data.ndim != 2:
            logger.warning(
                f"Warning: Invalid data dimensions in {image_path}. Expected 2D, got {img_data.ndim}.")
            return None
...
def _estimate_exposure_adjustment_from_data(image_data: np.ndarray) -> float:
    ...
    try:
        ...
        if finite_data.size == 0:
            logger.warning(
                "Warning: Exposure adjustment based on image with no finite pixels. Returning neutral.")
            return 1.0

        current_median = float(np.median(finite_data))

        target_8bit = int(np.clip(capture_cfg.get(
            'target_brightness', 128), 1, 254))
...
def _detect_aircraft_from_data_dry_run(image_data: np.ndarray, original_shape: Tuple[int, int], sim_initial_error_s: float = 0.0) -> Dict:
    """Dry run aircraft detection logic."""
    h, w = original_shape
    sharpness = _measure_sharpness_from_data(
        image_data)  # Calculate real sharpness
...
def _detect_aircraft_from_data(image_data: np.ndarray, original_shape: Tuple[int, int], sim_initial_error_s: float = 0.0) -> Dict:
    ...
    if CONFIG['development']['dry_run']:
        return _detect_aircraft_from_data_dry_run(image_data, original_shape, sim_initial_error_s)
    ...
    try:
        ...
        sharpness = _measure_sharpness_from_data(
            image_data)  # Uses patched function
        ...
        img_8bit_detect = cv2.normalize(np.nan_to_num(
        ...
        if current_max_dim > max_dim_detect:
            ...
            if (target_w > 0) and (target_h > 0):
                ...
                logger.warning(
                    f"Warning: Image scaling resulted in zero dimension (scale={scale}). Using original.")
                scale = 1.0
        ...
        otsu_thresh_val, thresh = cv2.threshold(
            img_8bit_scaled, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        ...
        if M.get('m00', 0) == 0:
            return {'detected': False, 'reason': 'invalid_moments_on_best', 'sharpness': sharpness}

        cx_s = M['m10'] / M['m00']
        cy_s = M['m01'] / M['m00']
        if scale == 0:
            return {'detected': False, 'reason': 'invalid_scale_factor', 'sharpness': sharpness}
...
def _save_png_preview_from_data(image_data: np.ndarray, png_path: str) -> str:
    ...
    try:
        ...
        if not success:
            raise IOError(
                f"Failed to write PNG file to {png_path} (cv2.imwrite returned False)")
        return png_path
    except Exception as e:
...
def measure_sharpness(image_path: str) -> float:
    ...
    if img_data is None:
        return 0.0
    # This now uses the patched (min-max) function
...
def save_png_preview(fits_path: str, png_path_out: Optional[str] = None) -> str:
    ...
    if not os.path.exists(fits_path):
        logger.warning(
            f"Warning: FITS file not found for PNG preview: {fits_path}")
        return ""
