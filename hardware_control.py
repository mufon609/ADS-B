...
class IndiController(PyIndi.BaseClient):
    ...
    def __init__(self):
        ...
        if is_dry_run:
            ...
            self.setServer(CONFIG['hardware']['indi_host'],
                           CONFIG['hardware']['indi_port'])
            if not self.connectServer():
                raise ConnectionError("Failed to connect to INDI server")
            logger.info("Waiting for devices...")
            time.sleep(5)

            self.mount = self.getDevice(
                CONFIG['hardware']['mount_device_name'])
            self.camera = self.getDevice(
                CONFIG['hardware']['camera_device_name'])
            self.focuser = self.getDevice(
                CONFIG['hardware']['focuser_device_name'])
            ...
            logger.info(
                f"  Setting BLOB mode for camera '{self.camera.getDeviceName()}' property '{self._blob_name}'")
            self.setBLOBMode(
                PyIndi.B_CLIENT, self.camera.getDeviceName(), self._blob_name)
            time.sleep(1)
            blob_prop = self.camera.getBLOB(self._blob_name)
            if blob_prop is None:
                logger.warning(
                    f"  - Warning: BLOB vector '{self._blob_name}' not available. Subscribing to all BLOBs.")
                self.setBLOBMode(
                    PyIndi.B_CLIENT, self.camera.getDeviceName(), None)
            ...
            for device, name in [(self.mount, "Mount"), (self.camera, "Camera"), (self.focuser, "Focuser")]:
                dev_name_str = getattr(
                    device, 'getDeviceName', lambda: 'UNKNOWN')()
                driver_info_prop = device.getText(
                    "DRIVER_INFO") if device else None
                driver_info_text = "Driver info not available."
                if driver_info_prop and len(driver_info_prop) > 0:
                    driver_info_text = getattr(
                        driver_info_prop[0], 'text', driver_info_text)
            ...
            if upload_mode:
                ...
                if widget and widget.s != PyIndi.ISS_ON:
                    self._set_switch_state(
                        upload_mode, "UPLOAD_CLIENT", PyIndi.ISS_ON)
            ...
            if cam_specs.get('cooling', {}).get('enabled', False):
                ...
                if cooler_switch:
                    ...
                    if widget and widget.s != PyIndi.ISS_ON:
                        self._set_switch_state(
                            cooler_switch, "COOLER_ON", PyIndi.ISS_ON)
                temp_prop = self._wait_number(self.camera, "CCD_TEMPERATURE")
                if temp_prop:
                    widget = temp_prop.findWidgetByName(
                    ...
                    if widget:
                        ...
                        if abs(widget.value - setpoint) > 0.1:
                            ...
                            logger.info(
                                f"  - Set Camera: Cooling enabled, setpoint {setpoint}°C.")
            ...
            if 'binning' in cam_specs:
                ...
                if bin_prop:
                    ...
                    if is_hor_bin_different or is_ver_bin_different:
                        ...
                        logger.info(
                            f"  - Set Camera: Binning to {bin_x}x{bin_y}.")
    ...
    def _detect_ccd_blob_name(self) -> str:
        ...
        if not self.camera:
            logger.error(
                "  - Error: Camera device not initialized during BLOB detection.")
        ...
        logger.warning(
            "  - Warning: Could not detect primary BLOB property, falling back to 'CCD1'.")
        return "CCD1"
    ...
    def newBLOB(self, bp):
        ...
        blob_data = None
        try:
            if len(bp) > 0 and bp[0].bloblen > 0:
                blob_data = bp[0].getblobdata()
            else:
                logger.warning("  - Warning: Received empty BLOB.")
                PyIndi.BaseClient.newBLOB(self, bp)
                return
        except Exception as e:
        ...
        with self._blob_lock:
            try:
                ...
                logger.error(
                    f"  - Error processing BLOB in newBLOB handler: {e}")
        PyIndi.BaseClient.newBLOB(self, bp)
    ...
    def get_hardware_status(self) -> dict:
        ...
        try:
            ...
            ) if not CONFIG['development']['dry_run'] else self.simulated_az_el
        except Exception as e:
        ...
        if not CONFIG['development']['dry_run']:
            ...
            if self.camera:
                ...
                if bin_prop:
                    ...
                    if w1 and w2:
                        status["camera_binning"] = {
                            "x": int(w1.value), "y": int(w2.value)}

                gain_prop = self.camera.getNumber("CCD_GAIN")
                if gain_prop:
                    widget = gain_prop.findWidgetByName("GAIN")
                    if widget:
                        status["camera_gain"] = int(widget.value)

                offset_prop = self.camera.getNumber("CCD_OFFSET")
                if offset_prop:
                    widget = offset_prop.findWidgetByName("OFFSET")
                    if widget:
                        status["camera_offset"] = int(widget.value)

            if self.focuser:
                pos_prop = self.focuser.getNumber("ABS_FOCUS_POSITION")
                if pos_prop:
                    widget = pos_prop.findWidgetByName(
                        "FOCUS_ABSOLUTE_POSITION")
                    if widget:
                        status["focuser_pos"] = int(widget.value)

                focuser_temp_prop = self.focuser.getNumber(
                    "FOCUSER_TEMPERATURE")
                if focuser_temp_prop:
                    widget = focuser_temp_prop.findWidgetByName(
                        "FOCUSER_TEMPERATURE_VALUE")
                    if widget:
                        status["focuser_temp"] = round(widget.value, 1)
    ...
    def _get_current_az_el_internal(self) -> tuple:
        """
        Internal helper to get current mount Az/El.
        Tries horizontal coordinates first, then falls back to equatorial.
        """
        if not self.mount:
            raise ValueError("Mount device not initialized")
        ...
        if eq:
            ...
            if ra_w and dec_w:
                ...
                if not hasattr(self, 'observer_loc'):
                    raise ValueError("Observer location not initialized")
                ra_dec = SkyCoord(ra=ra*u.hourangle,
                                  dec=dec*u.deg, frame='icrs')
                altaz_frame = AltAz(obstime=Time.now(),
                                    location=self.observer_loc)
    ...
    def slew_to_az_el(self, az: float, el: float, progress_cb: Optional[Callable[[float, float, str], None]] = None) -> bool:
        ...
        try:  # Sun safety check
            ...
            est_slew_time = slew_time_needed(
                (current_az, current_el), (az, el), max_slew, now_frame) if max_slew > 0 else 10.0
            arrival_time_unix = time.time() + est_slew_time + 2.0
            sun_az_el_future = get_sun_azel(
                arrival_time_unix, self.observer_loc)
            arrival_frame = AltAz(obstime=Time(
                arrival_time_unix, format='unix'), location=self.observer_loc)
            separation_deg_future = angular_sep_deg(
            ...
            logger.warning(
                f"Warning: Predictive sun safety check failed: {e}.")
        ...
        if alt_limit_prop:  # Altitude limit check
            ...
            if max_el_w and el > max_el_w.value:
                logger.warning(
                    f"SAFETY ABORT: El {el:.2f} > limit {max_el_w.value:.2f} deg.")
                if progress_cb:
                    try:
                        progress_cb(
                            *self._get_current_az_el_internal(), "abort")
            ...
            if eq:
                ...
                try:
                    ...
                    target_altaz = SkyCoord(
                        az=az*u.deg, alt=el*u.deg, frame=altaz_frame)
                ...
                logger.error(
                    "Error: No HORIZONTAL_COORD or EQUATORIAL_EOD_COORD available.")
                if progress_cb:
                    try:
                        progress_cb(
                            *self._get_current_az_el_internal(), "abort")
        ...
        if final_state == PyIndi.IPS_OK:  # SUCCESS Check
            ...
            logger.warning(
                f"Warning: Slew unexpected state ({state_str}). Failure.")
            if progress_cb:
                try:
                    progress_cb(*self._get_current_az_el_internal(), "abort")
                except Exception as e:
    ...
    def autofocus(self, alt_ft: float = None) -> bool:
        ...
        if not self.focuser or not self.camera:
            logger.error("Error: Focuser or Camera not connected.")
            return False
        ...
        if not abs_pos_prop:
            logger.error("Focus failed: No absolute position.")
            return False
        pos_widget = abs_pos_prop.findWidgetByName("FOCUS_ABSOLUTE_POSITION")
        if not pos_widget:
            logger.error("Focus failed: Cannot find position widget.")
            return False

        current_pos = pos_widget.value
        focuser_min, focuser_max = pos_widget.min, pos_widget.max
        logger.info(
            f"  Focuser limits: [{focuser_min:.0f} - {focuser_max:.0f}], Current: {current_pos:.0f}")

        step_base = int(af_cfg.get('step_base', 50))
        scan_range = int(af_cfg.get('scan_range', 5))
        step_size = step_base
        raw_positions = [current_pos + i *
                         step_size for i in range(-scan_range, scan_range + 1)]
        positions = sorted(list(
            set(int(round(p)) for p in raw_positions if focuser_min <= p <= focuser_max)))

        if not positions or len(positions) < 3:
            logger.error(
                f"Focus failed: Insufficient positions ({len(positions)}).")
            return False
        ...
        for pos in positions:  # Scan loop
            ...
            if final_state != PyIndi.IPS_OK:  # Check completion
                state_str = {PyIndi.IPS_IDLE: "IDLE", PyIndi.IPS_OK: "OK", PyIndi.IPS_BUSY: "BUSY",
                             PyIndi.IPS_ALERT: "ALERT"}.get(final_state, str(final_state))
                logger.error(
                    f"  Focus move to {pos} failed (State: {state_str}). Aborting scan.")
                return False

            snap_path = None  # Capture image
            try:
                snap_path = self.capture_image(focus_exposure, os.path.join(
                    LOG_DIR, 'images', f"autofocus_{pos}_{time.time_ns()}.fits"))
                if not snap_path:
                ...
                logger.error(f"  Error capture/analysis at {pos}: {e}")
                sharpness_scores[pos] = 0.0

        if not sharpness_scores:
            logger.error("Focus failed: No scores recorded.")
            return False
        ...
        if best_sharp < sharp_thresh:
            logger.error(
                f"Focus failed: Best sharpness ({best_sharp:.2f}) < threshold ({sharp_thresh:.1f}).")
            pos_widget.value = current_pos
            self.sendNewNumber(abs_pos_prop)
            return False

        logger.info(
            f"  Best focus at {best_pos:.0f} (Sharpness: {best_sharp:.2f}). Moving...")
        ...
        if final_state != PyIndi.IPS_OK:
            state_str = {PyIndi.IPS_IDLE: "IDLE", PyIndi.IPS_OK: "OK", PyIndi.IPS_BUSY: "BUSY",
                         PyIndi.IPS_ALERT: "ALERT"}.get(final_state, str(final_state))
            logger.warning(
                f"Warning: Move to best focus {best_pos} failed (State: {state_str}).")

        logger.info(f"Autofocus complete. Final position: {best_pos:.0f}")
        log_entry = {'alt_ft': alt_ft, 'best_pos': best_pos, 'sharpness': best_sharp,
                     'timestamp': time.time(), 'scan_data': sharpness_scores}
        append_to_json([log_entry], os.path.join(LOG_DIR, 'focus_logs.json'))
        return True
    ...
    def snap_image(self, label: str) -> Optional[str]:
        ...
        if label.startswith("autofocus"):
            exp = float(CONFIG['capture'].get(
                'autofocus', {}).get('exposure_s', 0.1))
        else:
            exp = float(CONFIG['capture'].get(
                'guiding', {}).get('guide_exposure_s', 0.1))
    ...
    def capture_sequence(self, icao: str, exposure_s: float, num_images: Optional[int] = None) -> List[str]:
        ...
        if num_images <= 0:
            return []
        logger.info(
            f"  Starting capture sequence of {num_images} images with exposure: {exposure_s:.4f}s")
        paths = []
        images_dir = os.path.join(LOG_DIR, 'images')
        for i in range(num_images):
            if self.shutdown_event and self.shutdown_event.is_set():
                ...
                break
            path = os.path.join(
                images_dir, f"capture_{icao}_{time.time_ns()}_{i+1}_of_{num_images}.fits")
            try:
                captured_path = self.capture_image(
                    exposure_s, path)  # Calls patched capture_image
                if captured_path:
                    paths.append(captured_path)
                else:
                    logger.error(f"  Frame {i+1}/{num_images} failed.")
            except Exception as e:
                logger.error(f"  Unexpected error capturing frame {i+1}: {e}")
                break
        if paths:
            try:
                cam_settings = {"gain": CONFIG['camera_specs'].get('gain'), "offset": CONFIG['camera_specs'].get(
                    'offset'), "binning": CONFIG['camera_specs'].get('binning', {"x": 1, "y": 1})}
                sequence_id = f"{icao}_{int(time.time())}"
                schedule_stack_and_publish(sequence_id, paths, cam_settings)
                logger.info(
                    f"  Scheduled stacking for burst {sequence_id} ({len(paths)} frames)")
            except Exception as e:
                logger.warning(f"Warning: could not schedule stacking: {e}")
        return paths

    def disconnect(self):
        """Disconnects from the INDI server."""
        if CONFIG['development']['dry_run']:
            logger.info("[DRY RUN] Simulating hardware disconnect.")
            return
        if self.isConnected():
