    ...
    try:
        ...
        try:
            ...
            if "cannot schedule new futures" in str(e):
                ...
                robust_stack, clipped_fraction = _sigma_clipped_mean(vol, clip_z)
        ...
        # Save FITS
        saved_products: Dict[str, Dict[str, str]] = {"mean": {}, "robust": {}, "anomaly": {}}
        try:
            saved_products["mean"]["fits"] = _write_fits(mean_fits_path, mean_stack_u16)
        except Exception as e:
            qc.setdefault("save_errors", []).append(f"Mean FITS: {e}")
        try:
            saved_products["robust"]["fits"] = _write_fits(robust_fits_path, robust_stack_u16)
        except Exception as e:
            qc.setdefault("save_errors", []).append(f"Robust FITS: {e}")
        try:
            saved_products["anomaly"]["fits"] = _write_fits(anomaly_fits_path, anomaly_scaled_u16)
        except Exception as e:
            qc.setdefault("save_errors", []).append(f"Anomaly FITS: {e}")
        # Save PNGs using unscaled floats for proper normalization
        try:
            saved_products["mean"]["png"] = _write_png(mean_png_path, mean_stack, 1.0, 99.0)
        except Exception as e:
            qc.setdefault("save_errors", []).append(f"Mean PNG: {e}")
        try:
            saved_products["robust"]["png"] = _write_png(robust_png_path, robust_stack, 1.0, 99.0)
        except Exception as e:
            qc.setdefault("save_errors", []).append(f"Robust PNG: {e}")
        try:
            saved_products["anomaly"]["png"] = _write_png(anomaly_png_path, anomaly, 5.0, 99.5)
        except Exception as e:
